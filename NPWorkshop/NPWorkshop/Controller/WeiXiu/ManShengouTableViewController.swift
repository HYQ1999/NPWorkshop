    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
       ...
       cell.userqx.text =  manshengouList.Manpeijianlist[indexPath.row].userqx
        ...
        override func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath) -> [UITableViewRowAction]? {
             manshengouList.loadData()
            let ok = UITableViewRowAction(style: .normal, title: "确认"){
                actionm, index in
                ...
                let okAction = UIAlertAction(title: "确定", style: .default, handler: {
                    ...
                    if numberOfMatches2 == 0
                    {
                        ...
                        let requesting : Models_MessagePeijian.Requesting = Models_MessagePeijian.Requesting(PartsAmount:newpw.text!,  PartsID: String(self.peijianID), ApplyUser:self.manshengouList.Manpeijianlist[indexPath.row].userid,UserID: self.userlist.userlist[indexPath.row].userid)
                        let alerttController = UIAlertController(title: "提示！", message: "通知成功！", preferredStyle: .alert)
                        let okkAction =  UIAlertAction(title: "好的" , style: .default , handler: nil )
                        alerttController.addAction(okkAction)
                        self.present( alerttController, animated:  true, completion: nil)
                        return
